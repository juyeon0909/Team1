...
const FridgeMain: React.FC = () => {
  ...
  const frozenItems = sortIngredients(filteredIngredients.filter(item => {
    const sType = (item.storagetype || item.type || '').toUpperCase();
    return sType === 'FROZEN' || sType === '냉동';
  }), froSort);

  const refrigeratedItems = sortIngredients(filteredIngredients.filter(item => {
    const sType = (item.storagetype || item.type || '').toUpperCase();
    return sType === 'REFRIGERATED' || sType === '냉장';
  }), refSort);

  const roomItems = sortIngredients(filteredIngredients.filter(item => {
    const sType = (item.storagetype || item.type || '').toUpperCase();
    return sType === 'ROOM_TEMP' || sType === '실온' || sType === '상온';
  }), roomSort);

  const renderSortSelect = (value: string, onChangeFn: (val: string) => void) => (
    <select value={value} onChange={(e) => onChangeFn(e.target.value)} className="room-sort-select">
    ...
    <div className="storage-page-container">
      <div className="storage-search-section">
        <div className="storage-search-box">
          ...
          <input type="text" placeholder="보관 중인 재료를 검색해보세요" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="storage-search-input" />
      ...
      <div className="storage-rooms-grid">
        ...
        <div className="storage-room-card">
          <div className="room-header frozen-theme room-header-flex">
            <span className="room-emoji"></span><h3 className="room-title">냉동</h3><span className="room-count">{frozenItems.length}</span>
            {renderSortSelect(froSort, setFroSort)}
          </div>
          <div className="room-list-scroll">
            {frozenItems.length === 0 ? <p className="empty-item-text">재료가 없습니다.</p> : frozenItems.map((item) => {
        ...
        <div className="storage-room-card">
          <div className="room-header refrigerated-theme room-header-flex">
            <span className="room-emoji"></span><h3 className="room-title">냉장</h3><span className="room-count">{refrigeratedItems.length}</span>
            {renderSortSelect(refSort, setRefSort)}
          </div>
          <div className="room-list-scroll">
            {refrigeratedItems.length === 0 ? <p className="empty-item-text">재료가 없습니다.</p> : refrigeratedItems.map((item) => {
              const finalName = item.itemname || item.name || '이름 없음';
              return (
                <div key={item.id} className={`storage-item-row ${getExpiryClass(item.expirationdate)}`}>
                  <span className="item-name-link" onClick={() => navigate(`/product/edit/${item.id}`)}>{finalName}</span>
        ...
        <div className="storage-room-card">
          <div className="room-header room-theme room-header-flex">
            <span className="room-emoji"></span><h3 className="room-title">상온</h3><span className="room-count">{roomItems.length}</span>
            {renderSortSelect(roomSort, setRoomSort)}
          </div>
          <div className="room-list-scroll">
            {roomItems.length === 0 ? <p className="empty-item-text">재료가 없습니다.</p> : roomItems.map((item) => {
              const finalName = item.itemname || item.name || '이름 없음';
              return (
                <div key={item.id} className={`storage-item-row ${getExpiryClass(item.expirationdate)}`}>
                  <span className="item-name-link" onClick={() => navigate(`/product/edit/${item.id}`)}>{finalName}</span>
